...
class CashfreeService {
  constructor() {
    ...
    this.returnUrl = process.env.CASHFREE_RETURN_URL || 'http://localhost:8000/payment-callback.html';
  ...
  async createOrder(data) {
    try {
      ...
      if (response.data && response.data.payment_session_id) {
        ...
        // Use payment_link from API response if available, otherwise construct URL
        const paymentUrl = response.data.payment_link || 
          (this.apiUrl.includes('sandbox')
            ? `https://payments-test.cashfree.com/order/#/checkout?order_id=${response.data.order_id}&order_token=${response.data.payment_session_id}`
            : `https://payments.cashfree.com/order/#/checkout?order_id=${response.data.order_id}&order_token=${response.data.payment_session_id}`);
