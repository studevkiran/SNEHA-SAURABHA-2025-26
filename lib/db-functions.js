...
// New format: GST00N0690, GST00N0691, GST00N0692, etc.
async function getNextRegistrationId() {
  if (!isDbAvailable) {
    ...
    return 'GST00N' + numPart.toString().padStart(4, '0');
  }
  
  try {
    // Query for HIGHEST number from BOTH old (2026RTY####) and new (GST00N####) formats
    // Extract last 4 digits from all registration IDs and find MAX
    const result = await pool.query(`
      SELECT registration_id,
             CAST(RIGHT(registration_id, 4) AS INTEGER) as num_part
      FROM registrations 
      WHERE registration_id ~ '^(2026RTY|GST00N)[0-9]{4}$'
    ...
    if (result.rows.length === 0) {
      ...
      console.log('⚠️ No registrations found, starting from GST00N0001');
      return 'GST00N0001';
    }
    ...
    const nextId = 'GST00N' + nextNumber.toString().padStart(4, '0');
    ...
    try {
      ...
      if (fallbackResult.rows[0].max_num) {
        ...
        return 'GST00N' + nextNum.toString().padStart(4, '0');
      }
    } catch (fallbackError) {
      console.error('❌ Fallback query also failed:', fallbackError);
    }
    
    console.log('⚠️ All queries failed, starting from GST00N0001');
    return 'GST00N0001';
  }
}
