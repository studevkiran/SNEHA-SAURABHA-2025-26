...
// New format: RAC54V0690, RAC54V0691, RAC54V0692, etc.
async function getNextRegistrationId() {
  if (!isDbAvailable) {
    ...
    return 'RAC54V' + numPart.toString().padStart(4, '0');
  }
  
  try {
    // Query for HIGHEST number from ALL formats (2026RTY####, GST00N####, RAC54V####)
    // Extract last 4 digits from all registration IDs and find MAX
    const result = await pool.query(`
      SELECT registration_id,
             CAST(RIGHT(registration_id, 4) AS INTEGER) as num_part
      FROM registrations 
      WHERE registration_id ~ '^(2026RTY|GST00N|RAC54V)[0-9]{4}$'
    ...
    if (result.rows.length === 0) {
      ...
      console.log('⚠️ No registrations found, starting from RAC54V0001');
      return 'RAC54V0001';
    }
    ...
    const nextId = 'RAC54V' + nextNumber.toString().padStart(4, '0');
    
    console.log(`🔢 Last ID: ${lastId} (${lastNumber}) → Next ID: ${nextId} (${nextNumber})`);
    return nextId;
  } catch (error) {
    ...
    try {
      ...
      if (fallbackResult.rows[0].max_num) {
        ...
        return 'RAC54V' + nextNum.toString().padStart(4, '0');
      }
    } catch (fallbackError) {
      console.error('❌ Fallback query also failed:', fallbackError);
    }
    
    console.log('⚠️ All queries failed, starting from RAC54V0001');
    return 'RAC54V0001';
  }
}
