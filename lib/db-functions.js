...
// Get next registration ID - SPT format for SPOT registrations
// Previous registrations ended at 2026RTY1197, now starting SPT001
async function getNextRegistrationId(registrationType, clubId, mealPreference) {
  // Use SPT prefix for spot registrations
  const prefix = 'SPT';
  ...
  if (!isDbAvailable) {
    ...
    const lastId = mockRegistrations.length > 0 
      ...
      : `${prefix}000`;
    const numPart = parseInt(lastId.slice(-3)) + 1;
    return prefix + numPart.toString().padStart(3, '0');
  }
  
  try {
    // Query for HIGHEST number from SPT registration IDs only
    // Extract ONLY last 3 digits and find MAX to continue SPT sequence
    ...
    if (result.rows.length === 0) {
      // No SPT registrations exist yet, start from 001
      console.log(`⚠️ No SPT registrations found, starting from ${prefix}001`);
      return `${prefix}001`;
    }
    ...
    console.log(`🔢 Last SPT ID: ${lastId} (${lastNumber}) → Next ID: ${nextId} (${nextNumber}) [Type: ${registrationType}, Prefix: ${prefix}]`);
    return nextId;
  } catch (error) {
