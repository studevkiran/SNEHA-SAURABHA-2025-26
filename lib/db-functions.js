...
// Old format: 2026RTY0001 to 2026RTY0689
// New format: PREFIX0690, PREFIX0691, PREFIX0692, etc.
// Prefix depends on registration type, club, meal preference
async function getNextRegistrationId(registrationType, clubId, mealPreference) {
  // Determine prefix based on registration type
  const prefixMap = {
    'rotarian': 'ROT54V',
    'rotarian with spouse': 'RWS54V',
    'ann': 'ANN54V',
    'annet': 'ANT54V',
    'guest': 'GST00N',
    'rotaractor': 'RAC54V',
    'silver donor': 'SLD54V',
    'silver sponsor': 'SLS54V',
    'gold sponsor': 'GLD54V',
    'platinum sponsor': 'PLT54V',
    'patron sponsor': 'PAT54V'
  };
  
  const typeKey = (registrationType || '').toLowerCase();
  const prefix = prefixMap[typeKey] || 'REG54V';
  ...
  try {
    // Query for HIGHEST number from ALL registration IDs (any prefix)
    // Extract last 4 digits and find MAX to continue sequence
    const result = await pool.query(`
      SELECT registration_id,
             CAST(RIGHT(registration_id, 4) AS INTEGER) as num_part
      FROM registrations 
      WHERE registration_id ~ '[0-9]{4}$'
    ...
    console.log(`🔢 Last ID: ${lastId} (${lastNumber}) → Next ID: ${nextId} (${nextNumber}) [Type: ${registrationType}]`);
    return nextId;
  } catch (error) {
    ...
    try {
      const fallbackResult = await pool.query(`
        SELECT MAX(CAST(RIGHT(registration_id, 4) AS INTEGER)) as max_num
        FROM registrations 
        WHERE registration_id ~ '[0-9]{4}$'
      `);
