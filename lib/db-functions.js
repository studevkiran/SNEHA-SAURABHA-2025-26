...
// Get next registration ID (continues sequence from old data)
// Unified format: 2026RTY#### for ALL registration types (continues from 0692)
async function getNextRegistrationId(registrationType, clubId, mealPreference) {
  // Use single unified prefix for all types
  ...
  if (!isDbAvailable) {
    ...
    const lastId = mockRegistrations.length > 0 
      ...
      : `${prefix}0000`;
    const numPart = parseInt(lastId.slice(-4)) + 1;
    return prefix + numPart.toString().padStart(4, '0');
  }
  
  try {
    // Query for HIGHEST number from ALL registration IDs
    // Extract ONLY last 4 digits and find MAX to continue sequence
    // This ensures we continue from 0689 (old data) regardless of prefix
    ...
    if (result.rows.length === 0) {
      // No registrations exist yet, start from 0001
      console.log(`⚠️ No registrations found, starting from ${prefix}0001`);
      return `${prefix}0001`;
    }
    
    const lastId = result.rows[0].registration_id;
    const lastNumber = result.rows[0].num_part;
    const nextNumber = lastNumber + 1;
    const nextId = prefix + nextNumber.toString().padStart(4, '0');
    
    console.log(`🔢 Last ID: ${lastId} (${lastNumber}) → Next ID: ${nextId} (${nextNumber}) [Type: ${registrationType}, Prefix: ${prefix}]`);
    return nextId;
  } catch (error) {
    console.error('❌ Error getting next registration ID:', error);
    // Fallback: try MAX on last 4 digits
    try {
      const fallbackResult = await pool.query(`
        SELECT MAX(CAST(SUBSTRING(registration_id FROM '.{4}$') AS INTEGER)) as max_num
        FROM registrations 
        WHERE LENGTH(registration_id) >= 4
          AND SUBSTRING(registration_id FROM '.{4}$') ~ '^[0-9]{4}$'
      `);
      ...
      if (fallbackResult.rows[0].max_num) {
        ...
        console.log(`🔄 Fallback: Using max ${fallbackResult.rows[0].max_num} → ${nextNum}`);
    ...
    console.log(`⚠️ All queries failed, starting from ${prefix}0001`);
    return `${prefix}0001`;
  }
}
...
async function createRegistration(data) {
  ...
  if (duplicateCheck.exists) {
    ...
    const errorMsg = `Mobile number ${data.mobile} is already registered. Registration ID: ${existing.registration_id}, Name: ${existing.name}, Type: ${existing.registration_type}`;
    console.error('❌ Duplicate mobile:', errorMsg);
    throw new Error(errorMsg);
  }

  const registrationId = await getNextRegistrationId(data.registrationType, data.clubId, data.mealPreference);
  ...
  try {
    console.log('💾 Attempting database INSERT with params:', {
      registrationId,
    ...
    // Resolve zone from club name if not explicitly provided
    const { getZoneForClub } = require('./zone-mapping');
    const clubNameForZone = data.clubName || '';
    const resolvedZone = data.zone && data.zone !== 'Unmapped' ? data.zone : getZoneForClub(clubNameForZone);
    const finalZone = resolvedZone || 'Unmapped';
    console.log('🗺️ Resolved zone for manual/bypass insert:', clubNameForZone, '→', finalZone);

    // 🔥 CRITICAL: Resolve club_id from database by club name
    // This prevents wrong club_ids from cached frontend data
    let finalClubId = data.clubId || 0;
    if (data.clubName && data.clubName !== 'Not Applicable') {
      try {
        const clubLookup = await pool.query(
          'SELECT id FROM clubs WHERE LOWER(TRIM(name)) = LOWER(TRIM($1)) LIMIT 1',
          [data.clubName]
        );
        if (clubLookup.rows.length > 0) {
          finalClubId = clubLookup.rows[0].id;
          console.log(`🔍 Server-side club_id lookup: "${data.clubName}" → ${finalClubId} (was ${data.clubId || 'null'})`);
        } else {
          console.warn(`⚠️ Club not found in database: "${data.clubName}" - using provided club_id: ${data.clubId || 0}`);
        }
      } catch (lookupError) {
        console.error('❌ Club lookup failed:', lookupError.message);
        // Fall back to provided club_id
      }
    }
    ...
    const result = await pool.query(
      ...
      [
        ...
        data.mobile,
        data.clubName,
        finalClubId, // Use server-resolved club_id instead of data.clubId
        finalZone,
    ...
    console.log('✅ Database INSERT successful:', result.rows[0].registration_id);
