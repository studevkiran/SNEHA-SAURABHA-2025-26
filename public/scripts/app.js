...
const EMBEDDED_CLUBS = [
    { "id": 1, "name": "Baikampady" },
    { "id": 2, "name": "Bajpe" },
    { "id": 3, "name": "Bannur" },
    { "id": 4, "name": "Bantwal" },
    { "id": 5, "name": "Bantwal Loretto Hills" },
    { "id": 6, "name": "Bantwal Town" },
    { "id": 7, "name": "B C Road City" },
    { "id": 8, "name": "Bellare Town" },
    { "id": 9, "name": "Belthangady" },
    { "id": 10, "name": "Birumale Hills Puttur" },
    { "id": 11, "name": "Central Mysore" },
    { "id": 12, "name": "Chamarajanagar" },
    { "id": 13, "name": "Chamarajanagar Silk City" },
    { "id": 14, "name": "Deralakatte" },
    { "id": 15, "name": "E-Club of Mysuru Center" },
    { "id": 16, "name": "Farangipete" },
    { "id": 17, "name": "Gonikoppal" },
    { "id": 18, "name": "H.D. Kote" },
    { "id": 19, "name": "Hemavathi Kodlipet" },
    { "id": 20, "name": "Heritage Mysuru" },
    { "id": 21, "name": "Hunsur" },
    { "id": 22, "name": "Ivory City Mysuru" },
    { "id": 23, "name": "Kadaba Town" },
    { "id": 24, "name": "Kinnigoli" },
    { "id": 25, "name": "Kollegal" },
    { "id": 26, "name": "Kollegal Mid Town" },
    { "id": 27, "name": "Krishnaraja" },
    { "id": 28, "name": "Krishnarajanagar" },
    { "id": 29, "name": "Kushalnagar" },
    { "id": 30, "name": "Madanthyar" },
    { "id": 31, "name": "Madikeri" },
    { "id": 32, "name": "Madikeri Woods" },
    { "id": 33, "name": "Malleshwara Alur Siddapura" },
    ...
    { "id": 64, "name": "Mysore Shreegandha" },
    { "id": 65, "name": "Mysore South East" },
    { "id": 66, "name": "Mysore Stars" },
    { "id": 67, "name": "Mysore West" },
    { "id": 68, "name": "Mysuru Diamond" },
    { "id": 69, "name": "Nanjangud" },
    { "id": 70, "name": "Panchsheel Mysore" },
    { "id": 71, "name": "Periyapatna Icons" },
    { "id": 72, "name": "Periyapatna Mid Town" },
    { "id": 73, "name": "Puttur" },
    { "id": 74, "name": "Puttur Central" },
    { "id": 75, "name": "Puttur City" },
    { "id": 76, "name": "Puttur-East" },
    { "id": 77, "name": "Puttur Elite" },
    { "id": 78, "name": "Puttur Swarna" },
    { "id": 79, "name": "Puttur Yuva" },
    { "id": 80, "name": "Seva Mysore" },
    { "id": 81, "name": "Shanivarsanthe" },
    { "id": 82, "name": "Siddakatte Phalguni" },
    { "id": 83, "name": "Somwarpet Hills" },
    { "id": 84, "name": "Subramanya" },
    { "id": 85, "name": "Sullia" },
    { "id": 86, "name": "Sullia City" },
    { "id": 87, "name": "Surathkal" },
    { "id": 88, "name": "Uppinangadi" },
    { "id": 89, "name": "Vijayanagar Mysore" },
    { "id": 90, "name": "Virajpet" },
    { "id": 91, "name": "Vittal" },
    { "id": 92, "name": "Yelanduru Greenway" }
];
...
async function fetchAndShowRegistration(orderId, autoDownload = false) {
    try {
        ...
        // If we reach here, payment is either successful or failed/not found (not pending)
        // Reset retry count as we are no longer in a pending state
        window.retryCount = 0;

        if (result.success && result.registration) {
            console.log('✅ Payment received! Processing your registration...');
            const reg = result.registration;
            console.log('✅ Registration verified:', reg.registration_id);
            ...
            // Auto-download receipt if requested (e.g., after payment success)
            if (autoDownload) {
                console.log('⬇️ Auto-downloading receipt in 1.5 seconds...');
                setTimeout(() => {
                    downloadReceiptPDF();
                }, 1500);
            }
        } else {
            console.error('❌ Registration not found or verification failed:', result);
            alert('Registration verification failed. Please contact support.');
            showScreen('screen-banner');
        }
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if ((paymentStatus === 'success' || paymentStatus === 'pending') && orderId) {
        ...
        // Pass true for autoDownload to trigger PDF download automatically
        fetchAndShowRegistration(orderId, true);
...
function renderHotels(hotels) {
    ...
    hotels.forEach(hotel => {
        ...
        // Handle image - use real image if available, fallback to placeholder
    ...
    modal.innerHTML = `
        ...
        ">
            ...
            <button onclick="document.getElementById('birthday-wishes-modal').remove();" 
                style="position: absolute; top: 8px; right: 8px; background: none; border: none; font-size: 24px; cursor: pointer; color: #666; line-height: 1; padding: 3px 8px; z-index: 1;">
                ×
            </button>
            
            <h2 style="color: #C41E3A; font-size: 16px; margin-bottom: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">
                Birthday Wishes
            </h2>
            ...
            <div style="background: linear-gradient(135deg, #D4AF37 0%, #C19A2E 100%); padding: 12px 10px; border-radius: 10px; margin: 12px 0; box-shadow: 0 4px 12px rgba(212, 175, 55, 0.3);">
                ...
                <div style="font-size: 11px; line-height: 1.4; margin-bottom: 8px; color: #FFFEF9;">
                    In celebration of our DG's Birthday, special offer for today only:
                </div>
                <div style="font-size: 12px; background: rgba(255, 255, 255, 0.2); padding: 8px; border-radius: 6px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 6px; border-bottom: 1px solid rgba(255, 255, 255, 0.3);">
                        <strong style="color: #FFFEF9; font-size: 12px;">Rotarian:</strong>
                        <div style="text-align: right;">
                            <span style="text-decoration: line-through; opacity: 0.75; color: #FFF3D4; font-size: 11px;">₹5,000</span>
                            <strong style="color: #FFF; font-size: 14px; margin-left: 6px; text-shadow: 0 1px 2px rgba(0,0,0,0.2);">₹4,500</strong>
                        </div>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 6px;">
                        <strong style="color: #FFFEF9; font-size: 12px;">With Spouse:</strong>
                        <div style="text-align: right;">
                            <span style="text-decoration: line-through; opacity: 0.75; color: #FFF3D4; font-size: 11px;">₹8,000</span>
                            <strong style="color: #FFF; font-size: 14px; margin-left: 6px; text-shadow: 0 1px 2px rgba(0,0,0,0.2);">₹7,500</strong>
                        </div>
                    </div>
                </div>
            </div>
            
            <p style="color: #666; font-size: 9px; font-style: italic; margin-top: 8px; line-height: 1.3;">
                Goodwill gesture to honor the occasion. Valid for today only.
            </p>
